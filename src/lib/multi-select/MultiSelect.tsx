...
const MultiSelect = (props: MultiSelectPropType): JSX.Element => {
  ...
  useEffect(() => {
    setList(options);
    const filteredIds = options.filter(item=> item.checked === true).map((item)=> item.id);
  ...
  const onMouseDown = (event: Event): void => {
    if (event.target instanceof Node && !modalRef?.current?.contains(event.target) && !searchBarRef?.current?.contains(event.target)) {
      // to close the dropdown modal on clicking outside the modal and the search bar
      setIsModalVisible(false)
      setShowAllChips(false);
      return;
    }
    if(event.target instanceof Node && searchBarRef?.current?.contains(event.target) && thresholdForBubble){
  ...
  return (
    ...
    >
      <div className={`${classes.box} ${hasError && classes.errorBorder}`} ref={searchBarRef} style={getStyles(Elements.InputBox, styles)}>
